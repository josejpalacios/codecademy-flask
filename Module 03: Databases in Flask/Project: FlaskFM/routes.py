...
@app.route('/profiles')
def profiles():
    current_users = User.query.all() # Task 16: change here to a database query
...
def profile(user_id):
   ...
   my_playlist = Playlist.query.get(user.playlist_id) # Task 17: change here to a database query
...
def add_item(user_id, song_id, playlist_id):
   ...
   if not exists(new_item, my_playlist.items):
      ...
      # Task 18: using db session add the new item
      db.session.add(new_item)
      # Task 19: increase the counter for the song associated with the new item
      song.n += 1
      # Task 20: commit the database changes here
      db.session.commit()
...
def remove_item(user_id, item_id):
   ...
   # Task 21: using db.session delete the item
...
def dashboard():
  ...
  if request.method == 'POST':
    new_song = Song(title=form.title.data, artist=form.artist.data, n=1) #Task 22: create a new song here
    # Task 23: add it to the database
    db.session.add(new_song)
    #commit to the database
    db.session.commit()
  else:
        flash(form.errors)
  unpopular_songs = Song.query.order_by(Song.n)  # Task 24: add the ordering query here
  songs = Song.query.all()
  return render_template('dashboard.html', songs = songs, unpopular_songs = unpopular_songs[:3], form = form)
